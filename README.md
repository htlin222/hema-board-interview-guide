...
| A5 平板版手冊（PDF） | 站內 `/booklet-a5/`；同一個 release | 同上，跟 A4 版一起建置 |
...
A5 平板版（`/booklet-a5/`）讀同一份 `src/booklet/`，不另外寫內容：每章從新的一頁開始、內容自動往下流、字比 A4 版大，一章大約兩頁。某章只多出一小截時，頁面會把那章字級調小一檔收回整數頁。改 A4 手冊的內容，A5 版重新建置就同步。
